 ...
 */
	...
	lazy_img : function(){
		...
		lazyNode.each(function(){
			var self = $(this);
			if(self.is('img')){
				self.attr('src','/Public/fanye/loading_large.gif');
			}else{
				// 把原来的图片预先保存下来
				...
				self.css({
					'background-image'	: 'url(/Public/fanye/loading_large.gif)',
 ...
 */
	//插件启动函数
 	plugin : function(){
		...
		// 音符飘逸
		$('#coffee_flow').coffee({
			steams				: ["<img src='http://weixiaoxinpic.qiniudn.com/Public/fanye/audio_widget_01@2x.png?i=22' />","<img src='http://weixiaoxinpic.qiniudn.com/Public/img/audio_widget_01@2x.png?i=22' />"], 
			steamHeight			: 100,
			steamWidth			: 44 
		});
